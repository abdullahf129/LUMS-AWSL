...
function AddCATalogue(){
    ...
    const handleImageChange = (file: File) => {
        setCATalogue({ ...CATalogue, img: [...CATalogue.img, file] });
      };
    ...
    const handleAddCATalogue = (event: any) => {
        event.preventDefault()

        let data = new FormData();

        for (let i = 0; i < CATalogue.img.length; i++) {
            data.append('CATalogueimg', CATalogue.img[i]);
        }

        data.append('name', CATalogue.name);
        data.append('age', CATalogue.age.toString());
        data.append('sex', CATalogue.sex);
        data.append('shortDescription', CATalogue.shortDescription);
        data.append('longDescription', CATalogue.longDescription);

    

        instance.post('/CATalogue/create', data, {
            headers: {
    ...
    return (
        <div>
            <h1 className='flex-1 text-xl md:text-4xl font-bold text-center mt-10 '>Adding a New Meow</h1>
                    ...
                    <div className='flex-1 relative'>
                        <div className='flex items-center space-x-3 bg-[#0000000A] rounded-lg border-2 px-5  py-2'>
                            ...
                            <input type='name' className='flex-1 bg-transparent focus:outline-none px-5 w-full' placeholder='Name'
                                onChange={(event) => setCATalogue({ ...CATalogue, name: event.target.value })} required
                ...
                <div className='flex items-center w-2/3'>
                    <div className='flex-1 relative'>
                        <div className='flex items-center space-x-3 bg-[#0000000A] rounded-lg border-2 px-5  py-2'>
                            ...
                            <input type='text' className='flex-1 bg-transparent focus:outline-none px-5 w-full' placeholder='Animal Sex'
                                onChange={(event) => setCATalogue({ ...CATalogue, sex: event.target.value })} required
                            />
                        </div>
                    </div>
                </div>
                    ...
                    <div className='flex-1 relative'>
                        <div className='flex items-center space-x-3 bg-[#0000000A] rounded-lg border-2 px-5  py-2'>
                            ...
                            <input type='number' className='flex-1 bg-transparent focus:outline-none px-5 w-full' placeholder='Age'
                                onChange={(event) => setCATalogue({ ...CATalogue, age: Number(event.target.value) })} required
                            />
                        </div>
                    </div>
                </div>
                    ...
                    <div className='flex-1 relative'>
                        <div className='flex items-center space-x-3 bg-[#0000000A] rounded-lg border-2 px-5  py-2'>
                            ...
                            <input type='text' className='flex-1 bg-transparent focus:outline-none px-5 w-full' placeholder='Short Description'
                                onChange={(event) => setCATalogue({ ...CATalogue, shortDescription: event.target.value })} required
                            />
                        </div>
                    </div>
                </div>
                    ...
                    <div className='flex-1 relative'>
                        <div className='flex items-center space-x-3 bg-[#0000000A] rounded-lg border-2 px-5  py-2'>
                            ...
                            <input type='text' className='flex-1 bg-transparent focus:outline-none px-5 w-full' placeholder='Long Description'
                                onChange={(event) => setCATalogue({ ...CATalogue, longDescription: event.target.value })} required
                            />
                        </div>
                    </div>
                </div>
